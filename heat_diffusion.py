...
hdargs.add_argument('heated_nodes',help='Nodes from which start the propagation and their initial heat value (from zero to one). Nodes of the network without value will be assigned a value of one.')
...
__,netformat=os.path.splitext(args.input_network)
...
if args.output_file is None:
 ...
 print(args.output_file,"is not NONE",os.path.dirname(args.output_file))
...
try:
 with open(args.heated_nodes) as inpf:
  heat=dict([line.split() for line in inpf])
except IOError as e:
...
if not any([k in nodes for k in heat.keys()]):sys.exit("ERROR! Not found any of the heated nodes in the network")
for node in nodes:
